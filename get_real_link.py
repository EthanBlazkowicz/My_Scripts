    ...
    try:
        ...
        if 300 <= response.status_code < 400:
            ...
            try:
                ...
                print(
                    f"{Colors.CYAN}[✓] Link successfully copied to clipboard!{Colors.RESET}"
                )
            except pyperclip.PyperclipException:
                print(
                    f"{Colors.YELLOW}[!] Could not copy to clipboard. Ensure 'xclip' or 'xsel' is installed.{Colors.RESET}"
                )

        else:
            print(
                f"{Colors.YELLOW}No redirect found. The provided link might already be the direct link:{Colors.RESET}"
            )
            print(url)
        ...
        print(
            f"{Colors.RED}Failed to resolve URL. Error: {e}{Colors.RESET}",
            file=sys.stderr,
        )
