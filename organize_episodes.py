...
def organize_tv_show_episodes():
    ...
    target_directory = input(
        "Enter the Target Directory (leave blank for current directory '.'): "
    ).strip()
    ...
    if not target_path.exists() or not target_path.is_dir():
        print(
            f"{Colors.RED}Error: The directory '{target_directory}' does not exist.{Colors.RESET}"
        )
        return
    ...
    if not season_folders:
        print(
            f"{Colors.YELLOW}No folders matching 'Season XX' found in '{target_directory}'.{Colors.RESET}"
        )
        return
    ...
    for folder, season_num in season_folders:
        ...
        all_files.sort(
            key=lambda x: [
                int(c) if c.isdigit() else c for c in re.split(r"(\d+)", x.name.lower())
            ]
        )
        ...
        unwanted_files = [
            ...
            and f.suffix.lower() not in sub_extensions
        ]
        ...
        for i in range(total_videos):
            ...
            print(
                f"{Colors.GREEN}Renaming Video: {video.name} -> {new_video_name}{Colors.RESET}"
            )
            ...
            if i < len(sub_files):
                ...
                print(
                    f"{Colors.DARK_GREEN}Renaming Subtitle: {sub.name} -> {new_sub_name}{Colors.RESET}"
                )
        ...
        if len(sub_files) > total_videos:
            print(
                f"{Colors.YELLOW}Warning: There are more subtitle files ({len(sub_files)}) than video files ({total_videos}). Some subtitles were left un-renamed.{Colors.RESET}"
            )
