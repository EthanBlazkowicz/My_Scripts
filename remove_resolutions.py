...
def remove_resolution_tags(directory, dry_run=False):
    ...
    pattern = re.compile(
        r"\s*(?:360p|720p|1080p|2160p|4k)(?=\.[^.]+$|$)", re.IGNORECASE
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Remove resolution tags (e.g., 1080p, 4K) from the end of filenames."
    )
    parser.add_argument(
        "directory",
        nargs="?",
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Perform a dry-run without renaming the files",
    )
