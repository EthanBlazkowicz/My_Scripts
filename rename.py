...
def load_dates_map():
    ...
    with open(DATES_FILE, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            match = re.search(
                r"(\d{4}[.-]\d{1,2}[.-]\d{1,2}).*?\b(No|NO|N0|HD)\.?\s*(\d+)\s*(.*)$",
                line,
                re.IGNORECASE,
            )
            if match:
                ...
                dates_map[num_key] = {"date": date_str, "rest": rest.strip()}
    return dates_map
...
def derive_video_name(filename, dates_map):
    ...
    if match:
        ...
        if (
            num_prefix.upper() == "HD"
            and len(num_digits) == 4
            and num_digits.startswith("0")
        ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Rename photo collection directories and video assets."
    )
    parser.add_argument(
        "path", nargs="?", default=os.getcwd(), help="Target operating path directory"
    )
    parser.add_argument(
        "-v",
        "--videos",
        action="store_true",
        help="Process and rename video files instead of directories",
    )
