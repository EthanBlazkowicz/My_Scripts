...
def parse_filename(filename):
    ...
    if resolution:
        base_name = re.sub(
            r"[\s_]*\(?" + resolution + r"\)?", "", base_name, flags=re.IGNORECASE
        )
        ...
        for a in soup.find_all("a", class_="result__url"):
            ...
            if title_el:
                ...
                if title_h2:
                    ...
                    title = re.sub(
                        r"\s*[-|]\s*" + re.escape(studio) + r".*",
                        "",
                        title,
                        flags=re.IGNORECASE,
                    )
                    title = re.sub(
                        r"\s*[-|]\s*Clips4sale.*", "", title, flags=re.IGNORECASE
                    )
                    title = title.strip()
...
def choose_best_match(query, results):
    ...
    if (
        highest_ratio > 0.7
    ):  # Lowered slightly to account for DuckDuckGo's "c**" censorship
        return best_match
...
def main():
    parser = argparse.ArgumentParser(
        description="Rename video files based on Clips4Sale search results."
    )
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Run without actually renaming files"
    )
    ...
    if not directory:
        prompt = (
            f"Enter the directory containing the video files [{default_dir}]: "
            if default_dir
            else "Enter the directory containing the video files: "
        )
    ...
    default_url = config.get(
        "last_url",
        "https://www.clips4sale.com/studio/68591/angel-the-dreamgirl/Cat0-AllCategories/Page1/C4SSort-added_at/Limit24/search",
    )
    base_url = args.url
    if not base_url:
