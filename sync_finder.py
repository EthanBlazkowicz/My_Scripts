...
def extract_audio_chunk(file_path, duration=60, sample_rate=16000):
    ...
    command = [
        "ffmpeg",
        "-ss",
        "0",  # Start at the absolute beginning
        "-i",
        file_path,  # Input path (Samba mount)
        "-t",
        str(duration),  # Stop reading after N seconds
        "-vn",  # Drop the video track completely
        "-ac",
        "1",  # Convert to Mono
        "-ar",
        str(sample_rate),  # Downsample to 16kHz for fast processing
        "-f",
    ...
    if process.returncode != 0:
        raise RuntimeError(
            f"FFmpeg failed reading {os.path.basename(file_path)}.\nError: {stderr.decode('utf-8', errors='ignore')}"
        )
...
def calculate_offset(
    ...
    duration=60,
    sample_rate=16000,
    output_txt="sync_offset_results.txt",
    append_mode=False,
):
    try:
        ...
        lags = scipy.signal.correlation_lags(
            len(audio_br_norm), len(audio_str_norm), mode="full"
        )
        ...
        if offset_seconds > 0:
            ...
            report += (
                "Result: Both files are perfectly aligned down to the millisecond!\n"
            )
        ...
        error_msg = (
            f"Sync analysis failed for {os.path.basename(file_streaming)}: {str(e)}"
        )
        print(error_msg)
        mode = "a" if append_mode else "w"
        with open(output_txt, mode, encoding="utf-8") as f:
...
def match_directories(bluray_dir, streaming_dir):
    ...
    br_files = [
        f for f in os.listdir(bluray_dir) if os.path.isfile(os.path.join(bluray_dir, f))
    ]
    str_files = [
        f
        for f in os.listdir(streaming_dir)
        if os.path.isfile(os.path.join(streaming_dir, f))
    ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Batch cross-correlate timeline offsets between folder tracks over SMB/Samba."
    )
    parser.add_argument(
        "-b",
        "--bluray_dir",
        required=True,
        help="Path to the directory containing Blu-ray source files",
    )
    parser.add_argument(
        "-s",
        "--streaming_dir",
        required=True,
        help="Path to the directory containing Streaming target files",
    )
    parser.add_argument(
        "-o",
        "--output",
        required=True,
        help="Path and name for the generated text report file",
    )
    parser.add_argument(
        "-d",
        "--duration",
        type=int,
        default=60,
        help="Duration of audio chunk to extract in seconds (default: 60)",
    )
    ...
    if not pairs:
        print(
            "Error: No matching episodes could be paired between directories based on file names."
        )
        exit(1)
    ...
    for idx, (br_path, str_path, ep_key) in enumerate(pairs):
        print(
            f"--- Processing Batch [{idx + 1}/{len(pairs)}]: Episode Key ({ep_key.upper()}) ---"
        )
        calculate_offset(
            file_bluray=br_path,
